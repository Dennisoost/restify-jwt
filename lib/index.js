...
module.exports = function(options) {
  if (!options || !options.secret) throw new Error({message: 'secret should be set'});
  ...
  var middleware = function(req, res, next) {
    ...
    async.parallel([
      ...
      if (revoked){
        return next(new restify.UnauthorizedError({ message:'The token has been revoked.' }));
      }

      var secret = results[0];

      jwt.verify(token, secret, options, function(err, decoded) {
        if (err && credentialsRequired) return next(new InvalidCredentialsError({message: String(err)}));
